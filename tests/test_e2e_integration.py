...
def test_full_integration_flow(mock_http_server, dummy_server_zip, valid_behavior_pack_zip):
    """
    ...
    server_zip_path = dummy_server_zip(target_dir=mock_http_server.directory, version="1.20.10.01")
    ...
    urllib.request.urlretrieve(f"{mock_http_server.url}/{server_zip_path.name}", downloaded_zip)
    
    downloaded_addon = download_dir / dest_addon_path.name
    urllib.request.urlretrieve(f"{mock_http_server.url}/{dest_addon_path.name}", downloaded_addon)
